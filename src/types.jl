...
* `varnames(ds)`: return a list of variable names as strings
...
"""
`AbstractVariable{T,N}` is a subclass of `DiskArrays.AbstractDiskArray{T,N}` which is a subclass of `AbstractArray{T, N}`.
A variable `v` of a type derived from `AbstractVariable` should implement:
...
* `DiskArrays.readblock!(v, aout, indices...)`: get the data of `v` at the provided indices. See [DiskArrays.jl readme](https://github.com/JuliaIO/DiskArrays.jl) for more details.
...
For a writable dataset, one should also implement:
* `defAttrib`: define a attribute
* `DiskArrays.writeblock!(v,ain,indices...)`: set the `ain` in `v` at the provided indices. See [DiskArrays.jl readme](https://github.com/JuliaIO/DiskArrays.jl) for more details.
